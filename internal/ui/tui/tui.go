...
func newModel(svc *app.Service, cfg *config.Config, log *slog.Logger) *model {
	...
	if strings.TrimSpace(cfg.GitHubToken) != "" {
		m.appendInfo(
			i18n.T(
				"GitHub token loaded (%s).",
				github.TokenType(cfg.GitHubToken),
			),
		)
	} else {
		m.appendWarn(
			i18n.T("GitHub token is not configured."),
		)
	}
...
func (m *model) appendResult(res *domain.Result) {
	...
	if isMarkdownMode(res.Mode) && strings.TrimSpace(res.Response) != "" {
		m.appendMarkdown(res.Response)
	} else if strings.TrimSpace(res.Response) != "" && res.Mode != "test" {
...
func (m *model) appendResultDetails(res *domain.Result) {
	...
	if res.Tests.Run {
		...
		// Детальное покрытие по пакетам — отдельным читаемым блоком.
		if strings.TrimSpace(res.Tests.CoverageOutput) != "" {
			m.appendInfo(i18n.T("Packet coverage:"))
			for _, line := range strings.Split(res.Tests.CoverageOutput, "\n") {
...
func (m *model) updateSuggestions() {
	...
	if first == ":agent" && (len(fields) > 1 || strings.HasSuffix(value, " ")) {
		if len(fields) > 2 {
			return
		}
		prefix := ""
		...
		agentSubs := []string{"deep", "interview", "reflect"}
	...
	// ─── Автодополнение для :help <раздел> ──────────────────────
	if first == ":help" && (len(fields) > 1 || strings.HasSuffix(value, " ")) {
		if len(fields) > 2 {
			return
		}
		prefix := ""
		...
		for _, topic := range app.HelpTopicNames() {
			if strings.HasPrefix(topic, prefix) {
				m.suggestions = append(m.suggestions, ":help "+topic)
			}
		}
		if len(m.suggestions) == 1 && m.suggestions[0] == strings.TrimSpace(value) {
			m.suggestions = nil
	...
	if first == ":autonomy" && (len(fields) > 1 || strings.HasSuffix(value, " ")) {
		if len(fields) > 2 {
			return
		}
		prefix := ""
		...
		autonomySubs := []string{"on", "off", "status", "run", "clear"}
		for _, sub := range autonomySubs {
			if strings.HasPrefix(sub, prefix) {
				m.suggestions = append(m.suggestions, ":autonomy "+sub)
			}
		}
		if len(m.suggestions) == 1 && m.suggestions[0] == strings.TrimSpace(value) {
			m.suggestions = nil
