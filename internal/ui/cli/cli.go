...
func Run(args []string, cfg *config.Config, log *slog.Logger, logPath string) error {
	...
	case "autonomy":
		return runAutonomy(rest, cfg, log)
	case "mutate":
		return runMutate(rest, cfg, log)
	case "autogen-tests":
		return runAutogenTests(rest, cfg, log)
	case "tui":
...
func runTaskFile(args []string, cfg *config.Config, log *slog.Logger) error {
	...
	// Интерактивный цикл для выбора подхода / ответов на вопросы
	res = handleAwaitingSelection(svc, res, *jsonOut, raw)
...
func runCode(args []string, cfg *config.Config, log *slog.Logger) error {
	...
	mode := fs.String(
		"mode",
		"",
		"execution mode: auto|simple|agent",
	)
	...
	if *agentMode {
		opts.Mode = "agent"
		opts.AgentDepth = app.AgentDepthNormal
	}

	if *deepMode {
		opts.Mode = "agent"
		opts.AgentDepth = app.AgentDepthDeep
	}
...
func applyCommonFlags(f commonFlags, cfg *config.Config) {
	...
	if apiKeyExplicit && f.key != nil && strings.TrimSpace(*f.key) != "" {
		fmt.Fprintln(os.Stderr, "[warn] API key passed via CLI flag; consider GOGITOR_API_KEY env var")
	}
	if githubKeyExplicit && f.githubKey != nil && strings.TrimSpace(*f.githubKey) != "" {
...
func printHelp() {
	fmt.Print(`gogitor — AI coding assistant for Go
...
// handleAwaitingSelection реализует интерактивный цикл для CLI-режима,
// когда результат требует выбора пользователя (сравнение подходов,
// интервью workflow, рецензирование плана).
// Если stdin не является терминалом (pipe), возвращает текущий результат без интеракции.
func handleAwaitingSelection(
	svc *app.Service,
	res domain.Result,
	jsonOut bool,
	raw bool,
) domain.Result {
	for res.AwaitingSelection {
		// Выводим текущий ответ (таблицу сравнения, вопросы и т.д.)
		printResult(res, jsonOut, raw)

		// Если stdin не терминал — интерактивный ввод невозможен
		if !isStdinTerminal() {
			return res
		}
		...
		// Читаем одну строку от пользователя
		input := readLineFromTerminal()
		if input == "" {
			// Пустой ввод — завершаем без выбора
			return res
		}

		// Передаём ввод в ProcessEvents, который обрабатывает
		// pendingComparison / pendingInterview / pendingPlanReview
		res = svc.ProcessEvents(context.Background(), input, emitCLI(jsonOut, raw))
	}
	return res
}

// readLineFromTerminal читает одну строку из терминального ввода.
...
func runAutonomy(args []string, cfg *config.Config, log *slog.Logger) error {
	...
	case "on":
		...
		res = domain.Result{Success: true, Mode: "autonomy", Response: "Autonomy monitor started."}
	case "off":
		svc.Autonomy.Monitor.Stop()
		res = domain.Result{Success: true, Mode: "autonomy", Response: "Autonomy monitor stopped."}
	case "run":
		res = svc.ProcessEvents(context.Background(), ":autonomy run", emitCLI(*jsonOut, raw))
	case "clear":
		svc.Autonomy.Queue = autonomy.NewTaskQueue()
		res = domain.Result{Success: true, Mode: "autonomy", Response: "Autonomy queue cleared."}
	default:
		res = svc.ProcessEvents(context.Background(), ":autonomy status", emitCLI(*jsonOut, raw))
	}
	printResult(res, *jsonOut, raw)
	return nil
}

func runMutate(args []string, cfg *config.Config, log *slog.Logger) error {
...
func runAutogenTests(args []string, cfg *config.Config, log *slog.Logger) error {
	fs := flag.NewFlagSet("autogen-tests", flag.ContinueOnError)
	common := registerCommonFlags(fs, cfg)
	jsonOut := fs.Bool("json", false, "print result as JSON")
	specs := buildFlagSpecs(
		flagSpec{Name: "json", Bool: true},
