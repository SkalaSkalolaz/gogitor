...
func InjectToken(repoURL, token string) string {
	...
	if strings.HasPrefix(repoURL, "git@") {
		...
		if len(parts) == 2 {
			return fmt.Sprintf("https://x-access-token:%s@%s/%s", token, parts[0], parts[1])
		}
	}
...
func (g *Git) WithAuthenticatedRemote(ctx context.Context, remote, token string, fn func() (string, error)) (string, error) {
	if token == "" {
		return fn()
	}
	g.authMu.Lock()
	...
	env := []string{
		"GIT_CONFIG_COUNT=1",
		fmt.Sprintf("GIT_CONFIG_KEY_0=remote.%s.url", remote),
		fmt.Sprintf("GIT_CONFIG_VALUE_0=%s", authURL),
	}
