...
//
// AUTONOMY MODE:
// :autonomy on         Start background monitoring (go build, go vet, TODO scan)
// :autonomy off        Stop background monitoring
// :autonomy status     Show monitor state and pending task queue
// :autonomy run        Execute fixable tasks from the queue (requires confirmation)
// :autonomy clear      Clear the task queue
//
// The autonomy monitor periodically runs:
// - go build (detects compilation errors)
// - go vet (detects static analysis issues)
// - TODO/FIXME scanner
//
// Found issues are queued as tasks. Use ':autonomy run' to auto-fix
// build errors and vet warnings via LLM. TODO items are shown as
// suggestions only.
//
// MUTATION TESTING:
// :mutate              Run mutation testing (default 20 mutations)
// :mutate 10           Run with custom mutation limit
//
// Deterministically generates operator mutations (>, >=, <, <=, &&, ||,
// ==, !=) in Go source files, applies each in a sandbox, runs tests,
// and reports which mutations survived (weak tests) vs killed (strong tests).
//
// AUTO TEST GENERATION:
// :autogen-tests       Generate tests for up to 5 untested functions
// :autogen-tests 3     Generate tests for up to 3 functions
//
// Scans the project AST for exported functions without corresponding
// _test.go files, generates table-driven tests via LLM, validates them,
// and keeps only passing tests.
...
// Autonomous engineering mode (background monitoring,
// task queue, auto-fix suggestions)
// :mutate [limit]      Run mutation testing on project (deterministic, no LLM)
// :autogen-tests [n]   Auto-generate unit tests for untested exported functions
...
- **:agent deep <task>**
    Run the strengthened Agent profile with:
    - task isolation
...
- **:agent interview <task>**
    Ask clarifying questions before running the Agent.
...
//
// АВТОНОМНЫЙ РЕЖИМ:
// :autonomy on         Запустить фоновый мониторинг (сборка, vet, TODO)
// :autonomy off        Остановить фоновый мониторинг
// :autonomy status     Показать состояние монитора и очередь задач
// :autonomy run        Выполнить исправляемые задачи из очереди
// :autonomy clear      Очистить очередь задач
//
// Фоновый монитор периодически выполняет:
// - go build (обнаружение ошибок компиляции)
// - go vet (обнаружение проблем статического анализа)
// - сканирование TODO/FIXME маркеров
//
// Найденные проблемы помещаются в очередь. Команда ':autonomy run'
// автоматически исправляет ошибки сборки и vet через LLM.
// Элементы TODO показываются только как предложения.
//
// МУТАЦИОННОЕ ТЕСТИРОВАНИЕ:
// :mutate              Запуск мутационного тестирования (по умолчанию 20 мутаций)
// :mutate 10           Запуск с пользовательским лимитом мутаций
//
// Детерминированно генерирует мутации операторов (>, >=, <, <=, &&, ||,
// ==, !=) в Go-файлах, применяет каждую в песочнице, запускает тесты
// и сообщает, какие мутации выжили (слабые тесты) или убиты (сильные).
//
// АВТОГЕНЕРАЦИЯ ТЕСТОВ:
// :autogen-tests       Генерация тестов для 5 нетестированных функций
// :autogen-tests 3     Генерация тестов для 3 функций
//
// Сканирует AST проекта на наличие экспортированных функций без
// соответствующих _test.go файлов, генерирует табличные тесты через
// LLM, проверяет их и сохраняет только проходящие.
//
// WORKFLOW:
//
...
- **:agent <задача>**
    Запустить полноценный Agent Harness.

- **:agent deep <задача>**
    Запустить усиленный профиль Agent:
    - изоляция подзадач
    - quality gates
    - артефакты сессии
    - финальная проверка
