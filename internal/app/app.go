...
func (s *Service) ProcessEvents(ctx context.Context, query string, emit func(domain.Event)) domain.Result {
    ...
    // Обработка pending workflow interview.
    if s.pendingInterview != nil {
        if time.Since(s.pendingInterview.CreatedAt) > 10*time.Minute {
            s.pendingInterview = nil
        } else {
            originalTask := s.pendingInterview.Task
            s.pendingInterview = nil
            return s.ContinueWorkflowInterview(ctx, originalTask, q, emit)
        }
    }

	// Обработка pending plan review.
	if s.pendingPlanReview != nil {
		if time.Since(s.pendingPlanReview.CreatedAt) > 10*time.Minute {
			s.pendingPlanReview = nil
		} else {
			return s.ContinueWorkflowPlanReview(ctx, q, emit)
		}
	}

	intent := s.resolveIntent(ctx, q, emit)
	task := effectiveTask(q, intent.Task)
...
func (s *Service) handleCommand(ctx context.Context, query string, emit func(domain.Event)) domain.Result {
    ...
    case ":workflow":
    	if argString == "" {
    		return domain.Result{Success: false, Mode: "code",
    			Errors: []string{"usage: :workflow <task> | :workflow interview <task> | :workflow reflect | :workflow pr"}}
    	}
    	lowerArgs := strings.ToLower(argString)
    
    	// ── Подкоманда: pr ────────────────────────────────────────
    	if lowerArgs == "pr" || lowerArgs == "mr" || lowerArgs == "pull-request" {
    		return s.ExecuteWorkflowPR(ctx, emit)
    	}
    
    	// ── Подкоманда: interview ─────────────────────────────────
    	if strings.HasPrefix(lowerArgs, "interview") || strings.HasPrefix(lowerArgs, "интервью") {
    		interviewTask := strings.TrimSpace(argString[len("interview"):])
    		if interviewTask == "" {
    			interviewTask = strings.TrimSpace(argString[len("интервью"):])
    		}
    		if interviewTask == "" {
    			return domain.Result{Success: false, Mode: "workflow-interview",
    				Errors: []string{"usage: :workflow interview <task>"}}
    		}
    		return s.ExecuteWorkflowInterview(ctx, interviewTask, emit)
    	}
    
    	// ── Подкоманда: reflect ───────────────────────────────────
    	if lowerArgs == "reflect" || lowerArgs == "рефлексия" || lowerArgs == "ретроспектива" {
    		return s.ExecuteWorkflowReflect(ctx, emit)
    	}
    
    	return s.ExecuteCode(ctx, argString, Options{Mode: "workflow"}, emit)
	...
	case ":agent":
		if argString == "" {
			return domain.Result{Success: false, Mode: "code", Errors: []string{"usage: :agent <task>"}}
		}
		return s.ExecuteCode(ctx, argString, Options{Mode: "agent"}, emit)
    case ":fix":
        if argString == "" {
            return domain.Result{
	...
	case ":git":
		...
		default:
			return domain.Result{
				Success: false,
				Mode:    "git",
				Errors:  []string{"unknown git command; supported: status, diff, commit, init, log, checkout, branch, merge, push, pull, fetch, clone, remote"},
...
func (s *Service) ExecuteCode(ctx context.Context, query string, opts Options, emit func(domain.Event)) domain.Result {
	...
	case ExecutionModeFast:
		sendEvent(emit, domain.EventAgent, "orchestrator disabled (fast mode)")
		sendEvent(emit, domain.EventAgent, "current stage: coder")
		sendEvent(emit, domain.EventAgent, "coder started")

		res := s.executeSimple(ctx, query, opts, emit)

		s.emitAgentBudget(emit, "coder")

		return res

	case ExecutionModeWorkflow:
		return s.executeWorkflowLite(ctx, query, opts, emit)

	case ExecutionModeAgent:
		// Принудительный agent режим falls through to agent branch below.
	case ExecutionModeAuto:
		// Автоматический режим использует старую эвристику ниже.
	}

	forceAgent := strategy.Mode == ExecutionModeAgent

	if (s.Cfg.MultiAgent || forceAgent) && (forceAgent || s.isComplex(query)) {
		sendEvent(emit, domain.EventIntent, "Complex task detected: multi-agent mode enabled")

		if s.Cfg.CompareApproaches && !opts.NoCompare {
			sendEvent(emit, domain.EventAgent, "current stage: approach comparison")
			sendEvent(emit, domain.EventLog, "Generating comparative analysis of approaches...")

			comparison, err := s.generateComparison(ctx, query, emit)
			if err == nil && comparison != nil && len(comparison.Approaches) > 0 {
				for i := range comparison.Approaches {
					if comparison.Approaches[i].ID == comparison.RecommendedID {
						comparison.Approaches[i].Recommended = true
					}
				}

				s.pendingComparison = &PendingComparison{
					Task:       query,
				...
				return domain.Result{
					...
					Response:          s.formatComparison(comparison),
				}
			}
			if err != nil {
				sendEvent(emit, domain.EventWarn,
					fmt.Sprintf("Comparison generation failed, proceeding normally: %v", err))
			}
		}

        return s.executeAgentFull(ctx, query, "", opts, emit)
    }

    sendEvent(emit, domain.EventAgent, "orchestrator disabled (simple task)")
    ...
    if !analysisOnly {
    	...
    	s.emitAgentBudget(emit, "analyze")
    }
...
func (s *Service) executeSimple(ctx context.Context, query string, opts Options, emit func(domain.Event)) domain.Result {
	...
	for i := 1; i <= maxIterations; i++ {
		...
		if len(changes) == 0 {
			if usePatchPrompt {
				lastErrors = []string{
					"LLM did not return valid patch/file blocks. Expected format: --- Patch: path --- or --- File: path ---",
				}
				forceFull = true
				sendEvent(emit, domain.EventWarn, "Patch mode failed: "+strings.Join(lastErrors, "; "))
				continue
			}
			lastErrors = []string{
		...
		if err := s.Runner.Build(ctx, sandbox); err != nil {
			...
			if patchModeChanges || usePatchPrompt {
				if patchFixAttempts < maxPatchFixAttempts {
					...
					sendEvent(emit, domain.EventWarn,
						"Patch applied but build failed. Will request patch fix.")
				} else {
...
// contextLimits возвращает лимиты контекста, масштабированные
// от размера контекста модели.
...
:workflow <task>           Execute task in workflow mode (with artifacts)
:workflow interview <task> Interview before workflow execution
:workflow reflect          Reflect on the last workflow session
:workflow pr               Create branch and PR from last workflow artifacts

WORKFLOW:
:workflow <task>           Execute a task in workflow mode with full traceability.
Creates a session directory under .gogitor/workflow/<timestamp>/
containing the following artifacts:
  inbox.md       — original request and clarified goal
  research.md    — project context gathered by the index
  plan.md        — implementation plan with acceptance criteria
  prd.json       — structured task list with statuses and commits
  process.md     — append-only execution log
  gate-report-task-XX.json — quality gate results per task
Behavior:
  1. Decomposes the task into 2–5 atomic subtasks.
  2. Executes each subtask via the coder agent.
  3. Runs quality gates after each task:
     go build, go test, go vet, gofmt -l, golangci-lint.
  4. If gates pass, commits the task separately
     (requires AutoGitCommit).
  5. If any gate fails, the workflow stops with an error.
Example:
  :workflow create a REST API with /health and /version endpoints

:workflow interview <task> Run a clarifying interview before executing the task.
Behavior:
  1. Sends the task to the LLM (planner role) and receives
     3–5 targeted clarifying questions with defaults.
  2. Displays the questions and waits for your answers.
  3. You can answer in the format "1: answer" per line,
     or type "skip" / "go" to accept all defaults.
  4. Based on your answers, the LLM refines the task.
  5. Starts the normal workflow with the refined task.
Use this when the task is ambiguous or underspecified.
Example:
  :workflow interview add caching to the API layer

:workflow reflect          Analyze the last completed workflow session.
Reads artifacts from the most recent .gogitor/workflow/<timestamp>/:
  inbox.md, process.md, prd.json, gate-report-task-*.json.
Behavior:
  1. Locates the latest workflow session directory.
  2. Reads all available artifacts.
  3. Sends them to the LLM for a retrospective analysis.
     For small local models a simplified prompt is used.
  4. Saves the result to reflection.md inside the session dir.
  5. Displays the reflection in the TUI / CLI output.
The reflection includes: what went well, what could improve,
metrics summary, recommendations, and a final verdict.
Example:
  :workflow reflect

:workflow pr               Create a Git branch and open a Pull Request
from the last workflow session artifacts.
Requirements:
  - A Git repository must exist (:git init if needed)
  - Remote 'origin' must be configured
  - GitHub token must be set (--key-github or GOGITOR_GITHUB_TOKEN)
Behavior:
  1. Finds the latest workflow session artifacts.
  2. Reads goal, plan.md, prd.json, process.md, gate reports.
  3. Builds a structured Markdown PR description containing:
     Goal, Implementation Plan, Tasks table (status/commit),
     Quality Gates table, Process Log (last 30 lines),
     Reflection (if available).
  4. Determines the branch strategy:
     - On base branch (main/master): creates workflow/<slug>
     - On an existing workflow/* branch: reuses it
     - On any other feature branch: reuses it
  5. Pushes the branch to origin.
  6. Creates a PR via the GitHub API into the default branch.
Example:
  :workflow pr
  :workflow pr --key-github ghp_xxx
...
:computer <task>     Execute a system administration task (computer mode)
                     Requires GOGITOR_COMPUTER_ENABLED=true
...
:git diff          Git diff
:git diff-task     Show cumulative diff of the last task
:git commit        Git commit all changes (single commit)
:git commit --split <f1,f2>
                   Create separate commits for specified files.
                   Each listed file gets its own LLM-generated message.
                   All remaining changes go into one general commit.
                   Aliases: --per-file, --by-file
                   Examples:
                     :git commit --split main.go
                     :git commit --split internal/git/git.go,internal/app/app.go
:git init          Initialize git repository
:git log           Show commit history (hash, date, message)
:git pr            Create a Pull Request from the current branch
:git issue         Create a GitHub Issue from failing tests
:git changelog     Generate CHANGELOG.md from Conventional Commits
:git pr-comment <n> [text]  Add a comment to a Pull Request
:git checkout <h>  Checkout a specific commit or branch
:git checkout -b <n> Create a new branch and switch to it
:git branch        List branches
:git branch <n>    Create a new branch
:git branch -d <n> Delete a branch
:git merge <n>     Merge a branch into current
:git revert [h]    Revert a commit (default: HEAD). Creates a NEW commit
                   that undoes the changes. Safe: does NOT rewrite history.
                   Examples:
                     :git revert          — revert the last commit
                     :git revert a3f7c21  — revert a specific commit
                   If a conflict occurs, resolve it and run ':git commit'.

:git reset [--hard] <h>
                   Reset the current branch to a specific commit.
                   WITHOUT --hard: uncommitted changes stay in working dir.
                   WITH --hard: ALL uncommitted changes are permanently lost.
                   Examples:
                     :git reset a3f7c21        — soft reset
                     :git reset --hard a3f7c21 — hard reset (destructive)
:git push [branch] Push to remote (GitHub)
:git pull [branch] Pull from remote (GitHub)
:git fetch         Fetch from remote
:git clone <url>   Clone a GitHub repository
:git remote        List remotes
:git remote add <name> <url>   Add remote
:git remote remove <name>      Remove remote
:git create <name> [--private] [--desc <text>] Create a new repository on GitHub.

TUI keys:
F2                 Toggle mouse text selection mode (copy LLM output)
...
:save <файл>       Сохранить последний результат в файл (.md, .txt, .go, .json)
...
:test lint         Запуск golangci-lint и автоисправление проблем через LLM
:suggest           Анализ состояния проекта: технический долг, отсутствующие тесты, запахи кода
:vet               Запуск go vet (быстро, без использования LLM)
:todo              Вывод TODO/FIXME/HACK маркеров в файлах проекта
:computer <задача> Выполнить задачу по управлению компьютером
                   Требует GOGITOR_COMPUTER_ENABLED=true
...
:workflow <задача>           Выполнить задачу в режиме workflow с полной трассируемостью.
Создаёт директорию сессии .gogitor/workflow/<timestamp>/
со следующими артефактами:
  inbox.md       — исходный запрос и уточнённая цель
  research.md    — контекст проекта, собранный по индексу
  plan.md        — план реализации с критериями приёмки
  prd.json       — структурированный список задач со статусами и коммитами
  process.md     — журнал выполнения (append-only)
  gate-report-task-XX.json — результаты quality gates для каждой задачи
Поведение:
  1. Разбивает задачу на 2–5 атомарных подзадач.
  2. Выполняет каждую подзадачу через агента-кодера.
  3. После каждой задачи запускает quality gates:
     go build, go test, go vet, gofmt -l, golangci-lint.
  4. Если gates пройдены — коммитит задачу отдельно
     (требует включённого AutoGitCommit).
  5. Если любой gate не пройден — workflow останавливается с ошибкой.
Пример:
  :workflow создать REST API с эндпоинтами /health и /version

:workflow interview <задача> Провести уточняющее интервью перед выполнением задачи.
Поведение:
  1. Отправляет задачу в LLM (роль planner) и получает
     3–5 целевых уточняющих вопросов с ответами по умолчанию.
  2. Показывает вопросы и ждёт ваших ответов.
  3. Отвечать можно в формате "1: ответ" по строкам,
     или ввести "skip" / "go" для принятия всех значений по умолчанию.
  4. На основе ответов LLM формирует уточнённую задачу.
  5. Запускает обычный workflow с уточнённой задачей.
Используйте, когда задача неоднозначна или недостаточно конкретна.
Пример:
  :workflow interview добавить кэширование в слой API

:workflow reflect          Проанализировать последнюю завершённую сессию workflow.
Читает артефакты из последней директории .gogitor/workflow/<timestamp>/:
  inbox.md, process.md, prd.json, gate-report-task-*.json.
Поведение:
  1. Находит последнюю директорию сессии workflow.
  2. Читает все доступные артефакты.
  3. Отправляет их в LLM для ретроспективного анализа.
     Для малых локальных моделей используется упрощённый промпт.
  4. Сохраняет результат в reflection.md внутри директории сессии.
  5. Выводит рефлексию в TUI / CLI.
Рефлексия включает: что прошло хорошо, что можно улучшить,
сводку метрик, рекомендации и итоговый вердикт.
Пример:
  :workflow reflect

:workflow pr               Создать Git-ветку и открыть Pull Request
из артефактов последней сессии workflow.
Требования:
  - Должен существовать Git-репозиторий (:git init при необходимости)
  - Remote 'origin' должен быть настроен
  - Токен GitHub должен быть задан (--key-github или GOGITOR_GITHUB_TOKEN)
Поведение:
  1. Находит артефакты последней сессии workflow.
  2. Читает цель, plan.md, prd.json, process.md, gate-отчёты.
  3. Формирует структурированное Markdown-описание PR:
     Цель, План реализации, Таблица задач (статус/коммит),
     Таблица Quality Gates, Лог процесса (последние 30 строк),
     Рефлексия (если доступна).
  4. Определяет стратегию ветвления:
     - На base-ветке (main/master): создаёт workflow/<slug>
     - На существующей ветке workflow/*: переиспользует её
     - На другой feature-ветке: переиспользует её
  5. Пушит ветку в origin.
  6. Создаёт PR через GitHub API в default branch.
Пример:
  :workflow pr
  :workflow pr --key-github ghp_xxx
...
:git status        Показать состояние рабочей директории.
                   Выводит список файлов в формате git status --short:
                     M  — файл изменён
                     A  — файл добавлен в индекс
                     D  — файл удалён
                     ?? — файл не отслеживается (новый, ещё не добавлен)
                   Если всё чисто, выводит "Working tree clean."
                   Используйте перед коммитом, чтобы увидеть,
                   какие файлы будут затронуты.

:git diff          Показать разницу (diff) между рабочей директорией
                   и последним коммитом (HEAD).
                   Показывает добавленные (+) и удалённые (-) строки
                   для каждого изменённого файла.
                   Если рабочая директория чистая (нет незакоммиченных
                   изменений), автоматически показывает diff последнего
                   коммита (HEAD~1..HEAD) или git show --stat.
                   Полезно для ревью изменений перед коммитом
                   или для понимания, что изменилось после последнего коммита.

:git diff-task     Показать накопительный diff последней задачи
                   (все изменения от начала задачи до конца,
                   включая все итерации и подзадачи агента)

:git commit        Закоммитить ВСЕ изменения в рабочей директории.
                   Эквивалент последовательности:
                     git add -A
                     git commit -m "gogitor: manual commit"
                   Если user.name / user.email не заданы в локальном
                   конфиге репозитория, автоматически устанавливает:
                     user.name  = Gogitor
                     user.email = gogitor@local
                   Если изменений нет, выводит "Nothing to commit."
                   При успехе показывает короткий хеш нового коммита.
                   Примечание: сообщение коммита фиксированное.
                   Для кастомного сообщения используйте git напрямую.

:git commit --split <ф1,ф2>
                   Создать раздельные коммиты для указанных файлов.
                   Каждый указанный файл получает отдельное сообщение,
                   сгенерированное LLM на основе его diff.
                   Все остальные изменения попадают в один общий коммит.
                   Алиасы: --per-file, --by-file
                   Примеры:
                     :git commit --split main.go
                     :git commit --split internal/git/git.go,internal/app/app.go

:git init          Инициализировать новый git-репозиторий в текущей
                   рабочей директории (эквивалент git init).
                   Создаёт скрытую папку .git со всей служебной
                   информацией репозитория.
                   Если репозиторий уже существует, команда завершается
                   без ошибок (повторная инициализация безопасна).
                   Используйте, когда начинаете новый проект и хотите
                   отслеживать историю изменений с самого начала.
                   После init можно сразу делать :git commit.

:git log           Показать историю коммитов (последние 20 записей).
                   Формат каждой строки:
                     <хеш> <дата> <сообщение>
                   Пример вывода:
                     a3f7c21 2026-07-25 gogitor: add HTTP handler
                     b8e1d04 2026-07-24 initial commit
                   Хеш можно использовать в :git checkout <хеш>
                   для возврата к конкретной версии проекта.
                   Если коммитов ещё нет, выводит "No commits yet."

:git changelog     Сгенерировать CHANGELOG.md из истории коммитов.
                   НЕ использует LLM — работает мгновенно и без модели.
                   Поведение:
                     1. Читает последние 200 коммитов через
                        git log --pretty=format:%s.
                     2. Парсит каждую строку как Conventional Commit:
                          type(scope): description
                     3. Группирует по типам:
                          🚀 Features     (feat)
                          🐛 Bug Fixes    (fix, hotfix)
                          ...
                          🏗️ Build        (build)
                          🔁 CI           (ci)
                          Other           (не Conventional Commits)
                     4. Записывает результат в CHANGELOG.md в корне
                        проекта (перезаписывает существующий).
                   Пример:
                     :git changelog
                   Рекомендация: используйте вместе с
                   :git commit (он уже генерирует Conventional Commits),
                   чтобы changelog всегда был осмысленным.

:git pr-comment <n> [текст]
                   Добавить комментарий к Pull Request по его номеру.
                   Требования:
                     - Настроен remote 'origin'
                     - Задан токен GitHub
                   Аргументы:
                     <n>    — номер PR (положительное целое, например 42)
                     [текст] — необязательный текст комментария.
                              Если не указан, используется стандартный:
                              "Automated review comment from Gogitor."
                   Поведение:
                     Отправляет POST
                       /repos/{owner}/{repo}/issues/{n}/comments
                     (PR и Issue используют одно API для комментариев).
                   Примеры:
                     :git pr-comment 42 "LGTM, можно мёржить"
                     :git pr-comment 42
                   Типичный сценарий: reviewer-агент нашёл замечания
                   во время мультиагентной задачи → вы одной командой
                   отправляете их как комментарий к PR, чтобы команда
                   увидела их прямо на GitHub.

:git checkout <h>  Переключиться на указанный коммит или ветку.
                   <h> — короткий или полный хеш коммита, либо имя ветки.
                   Примеры:
                     :git checkout a3f7c21   — перейти к коммиту a3f7c21
                     :git checkout main      — переключиться на ветку main
                     :git checkout feature   — переключиться на ветку feature
                   При переходе на коммит (не ветку) вы попадаете
                   в состояние "detached HEAD": вы видите код на момент
                   этого коммита, но новые коммиты не привязаны к ветке.
                   Чтобы вернуться: :git checkout main (или имя вашей ветки).
                   Если хеш не указан, команда покажет список последних
                   10 коммитов и подсказку по использованию.
                   Внимание: убедитесь, что нет незакоммиченных изменений,
                   иначе git может отказать в переключении.
...
:git branch <имя>  Создать новую ветку (без переключения на неё).
                   Пример: :git branch feature-auth
                   Для создания И переключения используйте:
                     :git checkout -b <имя>  (через git напрямую)

:git branch -d <и> Удалить ветку. Откажет, если ветка не слита.
                   Используйте -D для принудительного удаления.
                   Пример: :git branch -d feature-auth

:git merge <имя>   Слить указанную ветку в текущую.
                   Пример: находясь в main, выполните
                     :git merge feature-auth
                   Если есть конфликты, git сообщит о них —
                   разрешите конфликты вручную и сделайте :git commit.
:git revert [хеш]  Отменить коммит (по умолчанию HEAD). Создаёт НОВЫЙ коммит,
                   который отменяет изменения. Безопасно: история не переписывается.
                   Примеры:
                     :git revert          — отменить последний коммит
                     :git revert a3f7c21  — отменить конкретный коммит
                   При конфликте разрешите его вручную и выполните ':git commit'.

:git reset [--hard] <хеш>
                   Откатить текущую ветку до указанного коммита.
                   БЕЗ --hard: незакоммиченные изменения остаются в рабочей директории.
                   С --hard: ВСЕ незакоммиченные изменения безвозвратно удаляются.
                   Примеры:
                     :git reset a3f7c21        — мягкий откат
                     :git reset --hard a3f7c21 — жёсткий откат (деструктивный)

GITHUB / УДАЛЁННЫЙ РЕПОЗИТОРИЙ:

  Удалённый репозиторий (remote) связывает ваш локальный проект с GitHub.
  Для push/pull нужен remote. Настраивается один раз через ':git remote add'
  или временно через флаги --github и --key-github.

  --github <url>       URL репозитория (https://github.com/пользователь/репо).
                       Используется для разовых операций без сохранения в config.
  --key-github <токен> Токен GitHub для аутентификации (вместо пароля).
                       Classic PAT:  ghp_xxxxxxxx  (Settings > Developer > Tokens)
                       Fine-grained: github_pat_xxxxxxxx
                       Токен используется ТОЛЬКО на время операции
                       и НЕ сохраняется в .git/config.

:git create <name> [--private] [--desc <text>]
                   Создать новый пустой репозиторий на GitHub.
                   --private  — сделать репозиторий приватным (доступен только вам).
                   --desc     — добавить краткое описание репозитория.
                   После создания автоматически настраивает remote 'origin'.

:git clone <url>   Клонировать удалённый репозиторий в подпапку текущей директории.
                   Автоматически настраивает remote 'origin'.
                   Пример: :git clone https://github.com/user/myproject
                   С токеном (для приватных): :git clone <url> --key-github ghp_xxx
                   После клонирования рабочая директория переключается
                   на созданную подпапку.

:git remote        Показать список всех настроенных remote (имя и URL).
:git remote add <имя> <url>
                   Добавить связь с удалённым репозиторием.
                   Обычно имя — 'origin': :git remote add origin https://...
:git remote remove <имя>
                   Удалить связь с remote (сам репозиторий на GitHub не удаляется).
                   Пример: :git remote remove origin
:git remote set-url <имя> <url>
                   Изменить URL существующего remote (если репозиторий перемещён).

:git push [ветка]  Отправить локальные коммиты на GitHub.
                   Первый раз: :git push (автоматически создаст ветку в origin).
                   Если remote не настроен, используйте флаги:
                   :git push --github https://github.com/user/repo --key-github ghp_xxx

:git pull [ветка]  Скачать изменения с GitHub и автоматически слить их с текущей веткой.
                   Используйте перед началом работы для синхронизации кода.
                   Внимание: могут возникнуть конфликты слияния.

:git fetch         Скачать новые коммиты с GitHub БЕЗ автоматического слияния.
                   Безопасный способ проверить, что изменилось на сервере, перед pull.
                   После fetch можно посмотреть разницу (:git diff) и слить вручную.

:git pr            Создать Pull Request из текущей ветки в основную
                   (default_branch) ветку репозитория на GitHub.
                   Требования:
                     - Настроен remote 'origin' (или --github <url>)
                     - Задан токен GitHub (--key-github ghp_xxx или
                       GOGITOR_GITHUB_TOKEN)
                     - Текущая ветка НЕ main и НЕ master
                   Поведение:
                     1. Определяет owner/repo из remote 'origin'.
                     2. Берёт текущую ветку как head, default_branch
                        репозитория как base.
                     3. Генерирует title и body PR через LLM на основе
                        реального diff (с fallback: если LLM не ответил
                        или вернул невалидный JSON, title = "feat: changes
                        from <ветка>", body = "Automated PR").
                     4. Отправляет POST /repos/{owner}/{repo}/pulls.
                   Пример:
                     :git pr
                     :git pr --github https://github.com/user/repo \
                             --key-github ghp_xxx
                   Примечание: при ошибке HTTP 422 проверьте, что в ветке
                   есть коммиты, которых нет в base-ветке.

:git issue         Создать GitHub Issue автоматически из результатов
                   падающих тестов.
                   Требования:
                     - Настроен remote 'origin'
                     - Задан токен GitHub
                   Поведение:
                     1. Запускает 'go test -v -cover ./...' в песочнице.
                     2. Если все тесты прошли — ничего не создаёт.
                     3. Если есть упавшие тесты — формирует Issue:
                          Title: "test: N test(s) failing"
                          Body: список тестов с указанием функции,
                                файла:строки и сообщения об ошибке,
                                плюс сырой вывод (обрезанный).
                     4. Добавляет метки 'bug' и 'tests'.
                     5. Отправляет POST /repos/{owner}/{repo}/issues.
                   Пример:
                     :git issue
                   Полезно после ночного прогона тестов или когда нужно
                   зафиксировать регрессию без ручного копирования вывода.

Клавиши TUI:

F2                 Режим выделения текста мышью (копирование вывода)
...
func (s *Service) resolveIntent(ctx context.Context, query string, emit func(domain.Event)) domain.Intent {
...
func effectiveTask(original, refined string) string {
