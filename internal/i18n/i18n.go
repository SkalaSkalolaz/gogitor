...
var messages = map[Lang]map[string]string{
	...
	RU: map[string]string{
		...
		"Result saved to: %s":                  "Результат сохранён в: %s",
		"No completed task yet.":               "Завершённых задач пока нет.",
		"The last task produced no Git diff.":  "Последняя задача не создала Git diff.",
		"Task history is empty.":               "История задач пуста.",
		"Auto-save failed: %v":                 "Автосохранение не удалось: %v",
		"Cannot read task history: %v":         "Не удалось прочитать историю задач: %v",

		// ─── TUI: заголовки блоков ─────────────────────────────────
		"── Task History ─────────────────────":     "── История задач ─────────────────────",
		"── Task Diff: +%d / -%d ─────────────────": "── Diff задачи: +%d / -%d ─────────────────",
		"── End Task Diff ─────────────────────":    "── Конец Diff задачи ─────────────────────",
		"Quality Gates": "Контроль качества",
		"... (%d more lines, use :git diff-task for full diff)": "... (ещё %d строк, используйте :git diff-task для полного diff)",

		// ─── Computer Mode ─────────────────────────────────────────
		"Detecting OS...":                                 "Определение ОС...",
		"Generating execution plan...":                    "Формирование плана выполнения...",
		"Step %d/%d: %s":                                  "Шаг %d/%d: %s",
		"Command failed, attempting error recovery...":    "Команда не выполнена, попытка восстановления...",
		"Recovery: %s → %s":                               "Восстановление: %s → %s",
		"Verifying task completion...":                    "Проверка завершения задачи...",
		"Computer task completed: %d steps executed.\n%s": "Задача выполнена: %d шагов завершено.\n%s",
		"computer mode is disabled; use --computer flag, set GOGITOR_COMPUTER_ENABLED=true, or \"computer_enabled\": true in .gogitor.json": "режим управления компьютером отключён; используйте флаг --computer, установите GOGITOR_COMPUTER_ENABLED=true или \"computer_enabled\": true в .gogitor.json",
		"usage: :computer <task>":                                                       "использование: :computer <задача>",
		"step %d FORBIDDEN: %s (%s)":                                                    "шаг %d ЗАПРЕЩЁН: %s (%s)",
		"step %d failed: %v":                                                            "шаг %d не выполнен: %v",
		"step %d failed with exit code %d":                                              "шаг %d не выполнен, код выхода %d",
		...
		"usage: :load <path/to/file.txt|file.md>": "использование: :load <путь/к/файлу.txt|файл.md>",

		"Task loaded from file: %s": "Задача загружена из файла: %s",

		"cannot read task file: %w": "не удалось прочитать файл задачи: %w",

		"task file path is a directory: %s": "путь к файлу задачи является директорией: %s",

		"task file must have .txt or .md extension: %s": "файл задачи должен иметь расширение .txt или .md: %s",

		"task file is too large (%d bytes), max %d bytes": "файл задачи слишком большой (%d байт), максимум %d байт",

		"task file is empty: %s":                                                                                 "файл задачи пуст: %s",
		...
		"Checking module through Go module proxy...":                     "Проверяю модуль через Go module proxy...",

		"Proxy retry failed.": "Proxy retry не помог.",

		"Auto-search: researching current module/import path...": "Auto-search: ищу актуальный module/import path...",

		"Dependency information passed to LLM for repair.": "Информация о зависимости передана LLM для исправления.",
		...
		"%s / %s | Enter send | Alt+Enter newline | Ctrl+A copy | PgUp/PgDn history | F2: select | Ctrl+C quit":   "%s / %s | Enter отправить | Alt+Enter новая строка | Ctrl+A копировать | PgUp/PgDn история | F2: выделение | Ctrl+C выход",
		...
		// ─── Workflow: общие ────────────────────────────────────────────
		"Workflow dry-run is not fully supported; falling back to simple execution":    "Dry-run в режиме workflow поддерживается не полностью; переключение на простое выполнение",
		"Cannot create workflow dir %s: %v; falling back to .gogitor/workflow":         "Не удалось создать директорию workflow %s: %v; используется .gogitor/workflow",
		"Cannot create fallback workflow dir %s: %v; falling back to simple execution": "Не удалось создать резервную директорию workflow %s: %v; переключение на простое выполнение",
		"Workflow artifacts dir: %s":                                  "Директория артефактов workflow: %s",
		"Cannot write inbox.md: %v":                                   "Не удалось записать inbox.md: %v",
		"Cannot write research.md: %v":                                "Не удалось записать research.md: %v",
		"PRD validation failed: %v; rebuilding fallback PRD":          "Валидация PRD не удалась: %v; пересоздание резервного PRD",
		"Cannot write plan.md: %v":                                    "Не удалось записать plan.md: %v",
		"Cannot write prd.json: %v":                                   "Не удалось записать prd.json: %v",
		"Running workflow quality gates":                              "Выполнение quality gates workflow",
		"Cannot write gate report: %v":                                "Не удалось записать отчёт quality gates: %v",
		"previous workflow tasks may already have been committed":     "предыдущие задачи workflow могли уже быть закоммичены",
		"changes from the failed task remain applied but uncommitted": "изменения из неудачной задачи применены, но не закоммичены",
		"workflow task %d commit failed: %v":                          "коммит задачи workflow %d не удался: %v",
		"Workflow completed %d task(s).":                              "Workflow завершил %d задач(у).",
		"Invalid workflow dir %s: %v; using .gogitor/workflow":        "Некорректная директория workflow %s: %v; используется .gogitor/workflow",

		// ─── Workflow: interview ────────────────────────────────────────
		"Generating clarifying questions...":                                     "Формирование уточняющих вопросов...",
		"Interview question generation failed: %v; proceeding without interview": "Не удалось сформировать вопросы: %v; продолжаем без интервью",
		"No clarifying questions needed.":                                        "Уточняющие вопросы не требуются.",
		"Interview questions ready.":                                             "Вопросы интервью готовы.",
		"Processing interview answers...":                                        "Обработка ответов интервью...",
		"Task refined based on interview answers.":                               "Задача уточнена на основе ответов интервью.",
		"Interview refinement failed: %v; using original task":                   "Не удалось уточнить задачу: %v; используется исходная задача",

		// ─── Workflow: PR ───────────────────────────────────────────────
		"not a git repository; run :git init first":                 "не git-репозиторий; выполните :git init",
		"no workflow artifacts found: %v":                           "артефакты workflow не найдены: %v",
		"Workflow artifacts: %s":                                    "Артефакты workflow: %s",
		"workflow goal not found in inbox.md":                       "цель workflow не найдена в inbox.md",
		"Creating branch '%s' from '%s'...":                         "Создание ветки '%s' из '%s'...",
		"cannot create branch: %v":                                  "не удалось создать ветку: %v",
		"Using existing branch '%s'":                                "Используется существующая ветка '%s'",
		"Using current branch '%s'":                                 "Используется текущая ветка '%s'",
		"Pushing branch '%s' to origin...":                          "Отправка ветки '%s' в origin...",
		"push failed: %v":                                           "ошибка отправки: %v",
		"Pull Request created: #%d\n%s\nTitle: %s\nBranch: %s → %s": "Pull Request создан: #%d\n%s\nЗаголовок: %s\nВетка: %s → %s",

		// ─── Workflow: usage ────────────────────────────────────────────
		"usage: :workflow <task> | :workflow interview <task> | :workflow reflect | :workflow pr": "использование: :workflow <задача> | :workflow interview <задача> | :workflow reflect | :workflow pr",
		"usage: :workflow interview <task>":                             "использование: :workflow interview <задача>",
		"Interview skipped by user, using original task with defaults.": "Интервью пропущено пользователем, используется исходная задача со значениями по умолчанию.",

		"gofmt gate skipped (small model profile)": "Проверка gofmt пропущена (профиль малой модели)",

		"golangci-lint gate skipped (small model profile)": "Проверка golangci-lint пропущена (профиль малой модели)",

		"Processing plan review feedback...": "Обработка обратной связи по плану...",

		"Refining plan based on user feedback...": "Корректировка плана на основе обратной связи пользователя...",

		"Plan refined based on feedback.": "План скорректирован на основе обратной связи.",

		"Extracting lessons from reflection...": "Извлечение уроков из ретроспективы...",
		// ─── Reasoning ────────────────────────────────────────────
		"Reasoning mode enabled.":                "Режим размышления включён.",
		"Reasoning mode disabled.":               "Режим размышления выключен.",
		"Reasoning: %s (effort: %s)":             "Размышление: %s (глубина: %s)",
		...
		"Autonomy queue cleared.":                    "Очередь автономии очищена.",
		"Autonomy queue is empty. No pending tasks.": "Очередь автономии пуста. Нет ожидающих задач.",
		"usage: :autonomy [on|off|status|run|clear]": "использование: :autonomy [on|off|status|run|clear]",

		// ─── Autonomy: monitor ────────────────────────────────────────
		"Autonomy monitor: stopped": "Монитор автономии: остановлен",
		"✓ Build: passing":          "✓ Сборка: успешна",
		"⚠ Build: FAILING":          "⚠ Сборка: ОШИБКА",
		"✓ go vet: clean":           "✓ go vet: замечаний нет",
		"⚠ go vet: issues found":    "⚠ go vet: обнаружены замечания",

		// ─── Autonomy: mutate ─────────────────────────────────────────
		"## Mutation Testing Report": "## Отчёт мутационного тестирования",
		"No mutations were tested (no applicable operators found or all errors).": "Мутации не тестировались (не найдено применимых операторов или все ошибки).",
		"### ✗ Survived mutations (weak tests)":                                   "### ✗ Выжившие мутации (слабые тесты)",
		"### ✓ Killed mutations (strong tests)":                                   "### ✓ Убитые мутации (сильные тесты)",
		"### ⚠ Errors": "### ⚠ Ошибки",

		// ─── Autonomy: test generation ────────────────────────────────
		"## Untested Exported Functions":                              "## Экспортированные функции без тестов",
		"Use `:autogen-tests` to generate tests for these functions.": "Используйте `:autogen-tests` для генерации тестов для этих функций.",
		"## Test Generation Results":                                  "## Результаты генерации тестов",

		// ─── Autonomy: queue ──────────────────────────────────────────
		"Use `:autonomy run` to execute fixable tasks.": "Используйте `:autonomy run` для выполнения исправляемых задач.",

		// ─── Autonomy: commands ───────────────────────────────────────
		"Generating mutations (deterministic, no LLM)...": "Генерация мутаций (детерминированно, без LLM)...",
...
var ruPatterns = []pattern{
	...
	{regexp.MustCompile(`^Auto-search: found (\d+) source\(s\)$`),
		"Автопоиск: найдено $1 источника."},
	{regexp.MustCompile(`^usage: :load <path/to/file\.txt\|file\.md>$`),
		"использование: :load <путь/к/файлу.txt|файл.md>"},
	{regexp.MustCompile(`^Task loaded from file: (.*)$`),
		"Задача загружена из файла: $1"},
	// ─── Reasoning ────────────────────────────────────────────
	...
	// ─── Autonomy: сообщения с параметрами ────────────────────────
	{regexp.MustCompile(`^Executing autonomy task #(\d+): (.*)$`),
		"Выполнение задачи автономии #$1: $2"},
	{regexp.MustCompile(`^Autonomy run: (\d+) task\(s\) processed, (\d+) applied\.$`),
		"Автономный запуск: обработано задач: $1, применено: $2."},
	{regexp.MustCompile(`^Generated (\d+) mutations\. Running mutation tests\.\.\.$`),
		"Сгенерировано мутаций: $1. Запуск мутационных тестов..."},
	{regexp.MustCompile(`^Mutation (\d+)/(\d+): (.+):(\d+) \((.+)\)$`),
		"Мутация $1/$2: $3:$4 ($5)"},
	{regexp.MustCompile(`^Scanning for untested exported functions \(AST, no LLM\)\.\.\.$`),
		"Сканирование экспортированных функций без тестов (AST, без LLM)..."},
	{regexp.MustCompile(`^Found (\d+) untested function\(s\)\. Generating tests\.\.\.$`),
		"Найдено функций без тестов: $1. Генерация тестов..."},
	{regexp.MustCompile(`^Generating test for (.+) \((.+):(\d+)\)\.\.\.$`),
		"Генерация теста для $1 ($2:$3)..."},
	{regexp.MustCompile(`^Test file created: (.+) — running tests\.\.\.$`),
		"Создан файл теста: $1 — запуск тестов..."},
	{regexp.MustCompile(`^Generated tests failed \((\d+) passed, (\d+) failed\)\. File removed\.$`),
		"Сгенерированные тесты не прошли ($1 пройдено, $2 упало). Файл удалён."},
	{regexp.MustCompile(`^Tests pass \((\d+) passed\)\. File kept: (.+)$`),
		"Тесты пройдены ($1 пройдено). Файл сохранён: $2"},
	{regexp.MustCompile(`^Autonomy monitor: running \(last check: (.+)\)$`),
		"Монитор автономии: запущен (последняя проверка: $1)"},
	{regexp.MustCompile(`^Autonomy Queue: (\d+) pending task\(s\)$`),
		"Очередь автономии: $1 ожидающих задач"},
	{regexp.MustCompile(`^\[APPLIED\] #(\d+) \[(.+)\] (.*)$`),
		"[ПРИМЕНЕНО] #$1 [$2] $3"},
	{regexp.MustCompile(`^\[FAILED\] #(\d+) \[(.+)\] (.*)$`),
		"[ОШИБКА] #$1 [$2] $3"},
	{regexp.MustCompile(`^\[SUGGESTED\] #(\d+) \[(.+)\] (.*)$`),
		"[ПРЕДЛОЖЕНО] #$1 [$2] $3"},
	{regexp.MustCompile(`^Mutation Score: ([0-9.]+)% \((\d+) killed / (\d+) survived / (\d+) total$`),
		"Оценка мутаций: $1% ($2 убито / $3 выжило / $4 всего"},
	{regexp.MustCompile(`^- ✓ (.+):(\d+) — killed \((.+)\)$`),
		"- ✓ $1:$2 — убита ($3)"},
	{regexp.MustCompile(`^- ✗ (.+):(\d+) — SURVIVED \((.+)\): (.*)$`),
		"- ✗ $1:$2 — ВЫЖИЛА ($3): $4"},
	{regexp.MustCompile(`^- ⚠ (.+):(\d+) — error: (.*)$`),
		"- ⚠ $1:$2 — ошибка: $3"},
	{regexp.MustCompile(`^- (.+) in (.+):(\d+)$`),
		"- $1 в $2:$3"},
	{regexp.MustCompile(`^usage: :autonomy \[on\|off\|status\|run\|clear\]$`),
		"использование: :autonomy [on|off|status|run|clear]"},
}
