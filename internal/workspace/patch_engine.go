...
// PatchPolicyForModel выбирает строгость применения patch
// исходя из provider/model.
// FuzzyMinConfidence из config применяется отдельно как ручной override.
func PatchPolicyForModel(provider, model string) PatchPolicy {
	p := strings.ToLower(strings.TrimSpace(provider))
	m := strings.ToLower(strings.TrimSpace(model))

	// Облачные / внешние OpenAI-compatible модели считаем сильными.
	...
	// Для локальной неизвестной модели безопаснее balanced,
	// чем advanced.
	return PatchPolicyBalanced
}
...
func applyOnePatchWithPolicy(
	...
	// Если модель указала Symbol, сначала ограничиваем область поиска.
	// Это позволяет однозначно применить одинаковый SEARCH в разных
	// функциях одного файла.
	if strings.TrimSpace(p.Symbol) != "" {
		...
		// Если символ указан, но SEARCH внутри него не найден,
		// намеренно НЕ ищем дальше fuzzy по всему файлу.
		// Иначе Symbol потеряет смысл как безопасный anchor.
		if err != nil {
			return "", fmt.Errorf(
				"symbol %q: %w",
				p.Symbol,
