...
func main() {
    ...
    if len(legacyArgs) > 0 && !strings.HasPrefix(legacyArgs[0], "-") {
    	// Сохраняем старую возможность передавать API key
    	// третьим позиционным аргументом.
    	cfg.APIKey = legacyArgs[0]
    	legacyArgs = legacyArgs[1:]
    }
    
    // Теперь разбираем флаги, расположенные после provider/model.
    if len(legacyArgs) > 0 {
    	remaining, exit := parseGlobalFlags(legacyArgs, cfg)
    	if exit {
    		return
    	}
    
    	// Для legacy-режима после provider/model не должно
    	// оставаться позиционных аргументов.
    	if len(remaining) > 0 {
    		fmt.Fprintf(
    			os.Stderr,
    			"error: unexpected argument(s): %s\n",
